...
function getEmbedSource(pptPath, slideIndex) {
    ...
    const safePath = encodeURI(pptPath);
    const absolutePptUrl = new URL(safePath, window.location.href).href;
    return `https://view.officeapps.live.com/op/embed.aspx?src=${encodeURIComponent(absolutePptUrl)}&wdSlideIndex=${slideIndex}`;
}

function canUseOfficeEmbed() {
...
function initProjectPptEmbeds() {
    const pptBlocks = document.querySelectorAll('.project-ppt');
    if (pptBlocks.length === 0) return;

    pptBlocks.forEach(block => {
        const pptFile = block.getAttribute('data-ppt-file');
        const iframe = block.querySelector('.ppt-embed');
        const prevBtn = block.querySelector('.ppt-prev');
        const nextBtn = block.querySelector('.ppt-next');
        const indicator = block.querySelector('.ppt-slide-indicator');

        if (!pptFile || !iframe || !prevBtn || !nextBtn || !indicator) return;

        let currentSlide = 1;
        const minSlide = 1;

        const updateView = () => {
            indicator.textContent = `Slide ${currentSlide}`;
            prevBtn.disabled = currentSlide === minSlide;
            iframe.src = getEmbedSource(pptFile, currentSlide);
        };

        if (!canUseOfficeEmbed()) {
            block.classList.add('ppt-unavailable');
            indicator.textContent = 'Preview unavailable locally';
            prevBtn.disabled = true;
            nextBtn.disabled = true;
            iframe.removeAttribute('src');
            iframe.setAttribute('title', 'PPT preview unavailable locally');
            iframe.setAttribute(
                'srcdoc',
                '<div style="font-family:Arial,sans-serif;display:flex;align-items:center;justify-content:center;height:100%;padding:20px;color:#334155;text-align:center;background:#f8fafc;border:1px solid #e2e8f0;">Live slide preview works on deployed HTTPS URL. Use "Open PPT" for local preview.</div>'
            );
            return;
        }
